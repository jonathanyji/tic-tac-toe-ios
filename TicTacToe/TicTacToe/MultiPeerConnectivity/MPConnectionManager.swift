...
//  TicTacToe
//
//  Created by Jonathan Yeo on 30/12/2025.
//
...
class MPConnectionManager: NSObject, ObservableObject {
    ...
    let myPeerId: MCPeerID // My device peer id
    let nearbyServiceAdvertiser: MCNearbyServiceAdvertiser
    let nearbyServiceBrowser: MCNearbyServiceBrowser
    var game: GameState?

    func setup(game: GameState) {
        self.game = game
    }
    ...
    func send(gameMove: MPGameMove) {
        if !session.connectedPeers.isEmpty {
            do {
                let data = try JSONEncoder().encode(gameMove)
        ...
        if let gameMove = try? JSONDecoder().decode(MPGameMove.self, from: data) {
            DispatchQueue.main.async {
                ...
                case .start:
                    break
                case .move:
                    if let row = gameMove.row, let col = gameMove.col {
                        self.game?.placeTile(row, col)
                    }
                case .reset:
